<template>
  <div class="main">
    ...
    <!-- add by hbq 2021/10/14 当为默认密码时，弹出密码修改画面，并登出系统 -->
    <password-modal ref="passwordmodal" @ok="passwordModalOk" @close="passwordModalOk"></password-modal>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
        ...
        this.validateFields([ 'username', 'password', 'inputCode' ], (err)=>{
          if(!err){
            ...
            this.Login(loginParams).then((res) => {
              //console.log("登录：result", res);
              if(res.message.indexOf("[isDefaultPwdFlag=true]") > 0 ){
                res.result.isDefaultPwdFlag=true
              }else{
                res.result.isDefaultPwdFlag=false
              }
              this.$refs.loginSelect.show(res.result);
              if(res.result.isDefaultPwdFlag==true){
                this.handleChangePassword(loginParams.username);
              }
              //console.log("登录：ok....");
            }).catch((err) => {
      ...
      // handleChangePassword,passwordModalOk add by hbq 2021/10/14 当为默认密码时，弹出密码修改画面，并登出系统
      handleChangePassword(username) {
        this.$refs.passwordmodal.show(username);
      },
      passwordModalOk() {
        this.model.password="";
        this.$store.dispatch('Logout'); //登出系统
        this.handleChangeCheckCode();        
