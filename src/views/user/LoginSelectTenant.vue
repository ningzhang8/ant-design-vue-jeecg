    ...
    methods:{
      ...
      show(loginResult){
        ...
        if(this.visible===false){
          ...
          //add by hbq 2021/10/14 当为默认密码时，不进行页面跳转，设定用户的信息为默认默默登陆标识，        
          //1.如果用户采用页面刷新，而不是关闭密码修改画面，是可以登录，在 src\main.js 强制登出,2步理论不会再再触发
          //2.所有在src\components\tools\UserMenu.vue 中会依isDefaultPwdFlag标识再次弹出密码修改画面
          if (loginResult.isDefaultPwdFlag){
             user.isDefaultPwdFlag = loginResult.isDefaultPwdFlag;
             Vue.ls.set(USER_INFO,user);
          }else{
            this.$emit('success')
          }                   
        }
