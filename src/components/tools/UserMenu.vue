...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      //add by hbq 2021/10/14 当为默认密码时,弹出密码修改画面
      let user = Vue.ls.get(USER_INFO);
      if(user.isDefaultPwdFlag){
        setTimeout(() => {
          this.updatePassword();
        }, 1000);
        //setTimeout(this.updatePassword,1000);        
      }      
      //console.log("Path=components\tools\UserMenu.vue;User...",user);
    },
    computed: {
      ...mapState({
        // 后台菜单
